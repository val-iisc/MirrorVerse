...
bpy.context.preferences.addons[
    "cycles"
].preferences.compute_device_type = args.device  # or "OPENCL"
...
def set_camera_location(cam_pt):
    ...
    x, y, z = (
        cam_pt  # sample_spherical(radius_min=1.5, radius_max=2.2, maxz=2.2, minz=-2.2)
    )
...
def get_calibration_matrix_K_from_blender(camera):
    ...
    K = np.asarray(((alpha_u, skew, u_0), (0, alpha_v, v_0), (0, 0, 1)), np.float32)
    return K
...
def save_images(object_file: str) -> None:
    ...
    if args.camera_type == "fixed":
        azimuths = (np.arange(args.num_images) / args.num_images * np.pi * 2).astype(
            np.float32
        )
        elevations = np.deg2rad(
            np.asarray([args.elevation] * args.num_images).astype(np.float32)
        )
        ...
        elevations = np.random.uniform(
            args.elevation_start, args.elevation_end, args.num_images
        )
    ...
    if args.camera_type == "random":
        ...
        save_pickle(
            [K, azimuths, elevations, distances, cam_poses],
            os.path.join(args.output_dir, object_uid, "meta.pkl"),
        )


if __name__ == "__main__":
    """ https://github.com/liuyuan-pal/syncdreamer?tab=readme-ov-file#preparation-for-training
